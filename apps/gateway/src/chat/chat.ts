...
chat.openapi(completions, async (c) => {
	// Parse JSON manually without strict validation
	let rawBody: any;
	try {
	...
	if (!validationResult.success) {
		// Try to log the client error to the database if we can get project info
		try {
				...
				if (apiKey) {
					...
					if (project) {
						const baseLogEntry = createLogEntry(
							...
							project,
							apiKey,
							undefined, // providerKeyId
							"unknown", // usedModel
							"unknown", // usedProvider
							rawBody.model || "unknown", // requestedModel
							undefined, // requestedProvider
							rawBody.messages || [], // messages
							rawBody.temperature,
							rawBody.max_tokens,
							rawBody.top_p,
							rawBody.frequency_penalty,
							rawBody.presence_penalty,
							rawBody.tools,
							rawBody.tool_choice,
							source,
						);
			...
			// Silently handle logging errors - don't let them affect the response
			console.error("Failed to log client validation error:", logError);
		}

		// Return error response for invalid request body
		return c.json(
			{
				error: {
					message: "Invalid request body",
